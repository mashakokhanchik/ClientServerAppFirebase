...
class NetworkManager {
    ...
    func getNews(onComplete: @escaping ([News]) -> Void, onError: @escaping (Error) -> Void) {
        ...
        let task = session.dataTask(with: urlConstructor.url!) { (data, response, error) in
            ...
            guard let news = try? JSONDecoder().decode(Response<News>.self, from: data).response.items else {
                onError(ServerError.failedToDecode)
                return
            }
            DispatchQueue.main.async {
